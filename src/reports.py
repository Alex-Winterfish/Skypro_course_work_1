...
from src.utils import file_path, get_data
...
@log('../logs/spending_by_category.log')
def spending_by_category(data_frame: pd.DataFrame, search_str:str,date: Optional[str] = None)-> pd.DataFrame:
    ...
    if date is not None:
        ...
        logger.info(f'Дата от пользователя не передана. Для отчета взята текущая дата {datetime.today().strftime('%d.%m.%Y')}')
    ...
    i = 0
    while datetime.strptime(data_frame.loc[:, 'Дата операции'][i][0:10], '%d.%m.%Y') > date_proc:
        i += 1
    j = 0
    while datetime.strptime(data_frame.loc[:, 'Дата операции'][j][0:10], '%d.%m.%Y') >= datetime(period_year, period_month, date_proc.day):
        j += 1
    logger.info(f'Произведена выборка в период с {date_proc.day}.{period_month}.{period_year} по {date_proc.strftime('%d.%m.%Y')}' )
    ...
    for k in range(i,j):# цикл для поиска транзакций
        if type(data_dict.get(k).get('Категория')) == str:
            search_pattern = re.search(f"{search_str}", data_dict.get(k).get("Категория"), flags=re.I)
...
if __name__ == '__main__':
    ...
    search_str = input('введите категорию: ')

    user_data = input('введите дату: ')

    data_f = spending_by_category(data, search_str, user_data)
