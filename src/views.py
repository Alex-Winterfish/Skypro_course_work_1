...
from src.reports import log_simple
from src.utils import get_data, date_split, get_spending, sort_spending
...
def user_period():
    ...
    period = input('Введите период для выборки \n'
                   ' M - месяц \n W - неделя \n Y - год '
...
def main(data_frame:pd.DataFrame):
    ...
    split_data = date_split(data_frame, date, period = period) #получаем дата фрейм по пользовательской дате

    sort_data = get_spending(split_data) #формируем словарь с ключ-парой Категория:Сумма операции

    json_answer = sort_spending(sort_data)

    output_data = json.loads(json_answer)
