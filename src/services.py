...
def user_search_discription(data_dict, search_str)->json:
    ...
    logger.info(f'Функция {user_search_discription.__name__} производит поиск по запросу {search_str}')
    for i in range(len(data_dict)):  # цикл для поиска транзакций
        ...
        if search_pattern is not None:
            ...
            continue
    if output_list == []:
        logger.error(f'{user_search_discription.__name__} не найдены транзакции по запросу {search_str}')
        ...
        logger.info(f'json ответ функции {user_search_discription.__name__}: {json_data}')

        return json_data

def user_search_phone_number(data_dict) -> json:
    """Функция выполняет поиск во входном списке словарей транзакций с номерами телофонов"""
    import json
    import re
    output_list = list()  # переменная для накопления найденных транзакций
    logger.info(f'{user_search_phone_number.__name__} производит поиск номеров телефонов')
    for i in range(len(data_dict)):  # цикл для поиска транзакций
        ...
        if search_pattern is not None:
            ...
            continue
    if output_list == []:
        logger.error(f'{user_search_phone_number.__name__} не найдены транзакции с номерами телефонов')
        return "транзакции не найдены"
    else:
        json_data = json.dumps(output_list, ensure_ascii=False)
        logger.info(f'json ответ функции {user_search_phone_number.__name__}: {json_data}')
        return json_data
